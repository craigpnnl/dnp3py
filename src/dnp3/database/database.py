...
class Database:
    ...
    def get_binary_inputs_range(
        self, start: int, stop: int
    ) -> list[BinaryInputPoint]:
    ...
    def get_binary_outputs_range(
        self, start: int, stop: int
    ) -> list[BinaryOutputPoint]:
        """Get binary outputs in index range [start, stop]."""
        return [
            point
            for index, point in sorted(self.binary_outputs.items())
            if start <= index <= stop
        ]

    def get_analog_inputs_range(
        self, start: int, stop: int
    ) -> list[AnalogInputPoint]:
    ...
    def get_counters_range(self, start: int, stop: int) -> list[CounterPoint]:
        ...
        return [
            point
            for index, point in sorted(self.counters.items())
            if start <= index <= stop
        ]

    def get_frozen_counters_range(
        self, start: int, stop: int
    ) -> list[FrozenCounterPoint]:
        """Get frozen counters in index range [start, stop]."""
        return [
            point
            for index, point in sorted(self.frozen_counters.items())
            if start <= index <= stop
        ]
    ...
    def get_class_counters(self, event_class: EventClass) -> list[CounterPoint]:
        ...
        return [
            point
            for point in self.get_all_counters()
            if point.config.event_class == event_class
        ]

    def get_class_frozen_counters(
        self, event_class: EventClass
    ) -> list[FrozenCounterPoint]:
